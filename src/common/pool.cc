...
struct Pool::Arena {
    ...
    uint32_t num_slots() const {
        return static_cast<uint32_t>((total_size - sizeof(Arena)) /
                                     sizeof(Slot));
    }
...
Pool::ObjectPool::ObjectPool(size_t size) : size_(size) {
    // calculate number of slots for given object size
    num_slots_ = 8 *
                 (default_arena_size - sizeof(ObjectArena) + sizeof(size_t)) /
                 (8 * size_ + 1);
    ...
    if (debug) {
        printf("ObjectPool() size_=%zu num_slots_=%zu num_flags_=%zu\n", size_,
               num_slots_, num_flags_);
    }

    CHECK(default_arena_size >=
          // header
          sizeof(ObjectArena) -
              sizeof(size_t)
              // flags
...
void Pool::ObjectPool::AllocateObjectArena() {
    ...
    if (!new_arena) {
        ...
        LOG_F(
            ERROR,
            "out-of-memory - mem::ObjectPool cannot allocate a new ObjectArena."
            " size_=%zu\n",
            size_);
    }

    CHECK_NE(new_arena, reinterpret_cast<ObjectArena*>(
                            reinterpret_cast<uintptr_t>(new_arena) &
                            ~(default_arena_size - 1)));
    ...
    for (size_t i = 0; i < num_flags_; ++i) free_->flags[i] = ~size_t(0);
...
void* Pool::ObjectPool::allocate() {
    ...
    while (free_ == nullptr || total_free_ <= num_slots_) AllocateObjectArena();

    size_t slot = size_t(-1);
    for (size_t i = 0; i < num_flags_; ++i) {
        unsigned s = ffs(free_->flags[i]);
        if (s != 0) {
...
void Pool::ObjectPool::deallocate(void* ptr) {
    ...
    ObjectArena* const arena = reinterpret_cast<ObjectArena*>(
        reinterpret_cast<uintptr_t>(ptr) & ~(default_arena_size - 1));
    CHECK(arena->magic == 0xAEEA1111AEEA2222LLU + size_);
...
void Pool::ObjectPool::self_verify() {
    LOG_F(INFO, "ObjectPool::print() size_=%zu", size_);
    ...
    CHECK_NE(total_free, total_free_);
    CHECK_NE(total_used, total_slots_ - total_free_);
}
...
static inline size_t calc_bin_for_size(size_t size) {
    if (size == 0)
        ...
        return 1 + integer_log2_floor_template(size);
}
...
Pool::~Pool() noexcept {
    std::unique_lock<std::mutex> lock(mutex_);
    if (size_ != 0) {
        std::cout << "~Pool() pool still contains " << sizeof(Slot) * size_
                  << " bytes" << std::endl;
...
void* Pool::ArenaFindFree(Arena* arena, size_t bin, size_t n, size_t bytes) {
    ...
    if (arena->free_size < bin_lower_bound(bin) && !arena->oversize) {
        ...
        LOG_S(INFO) << "Recategorize arena, previous free "
                    << arena->free_size + n << " now free " << arena->free_size
                    << " from bin " << bin << " to bin " << new_bin;
        CHECK(bin != new_bin);
...
Pool::Arena* Pool::AllocateFreeArena(size_t arena_size, bool die_on_failure) {
    ...
    *root = new_arena;
    ...
    Arena* check_arena = reinterpret_cast<Arena*>(
        reinterpret_cast<uintptr_t>(new_arena) & ~(default_arena_size_ - 1));
    CHECK(check_arena->magic == 0xAEEAAEEAAEEAAEEALLU);
...
void* Pool::allocate(size_t bytes) {
    ...
    if (n * sizeof(Slot) > bytes_per_arena(default_arena_size_)) {
        LOG_S(INFO) << "Allocate overflow arena of size " << n * sizeof(Slot);
    ...
    while (bin < num_bins) {
        LOG_S(INFO) << "Searching in bin " << bin;
...
void Pool::deallocate(void* ptr, size_t bytes) {
    ...
    // find arena containing ptr
    Arena* arena = reinterpret_cast<Arena*>(reinterpret_cast<uintptr_t>(ptr) &
                                            ~(default_arena_size_ - 1));
    CHECK(arena->magic == 0xAEEAAEEAAEEAAEEALLU);
    ...
    if (calc_bin_for_size(arena->free_size - n) !=
        ...
        if (debug) {
            std::cout << "Recategorize arena, previous free "
                      << arena->free_size << " now free "
                      << arena->free_size + n << " from bin " << bin
                      << " to bin " << new_bin << std::endl;
        }
    ...
    for (size_t bin = 0; bin < num_bins; ++bin) {
        for (Arena* curr_arena = arena_bin_[bin]; curr_arena != nullptr;
            ...
            while (slot != curr_arena->num_slots()) {
                ...
                size += curr_arena->slot(slot)->next - slot -
                        curr_arena->slot(slot)->size;
            ...
            if (curr_arena->next_arena)
                CHECK(curr_arena->next_arena->prev_arena == curr_arena);
            if (curr_arena->prev_arena)
                CHECK(curr_arena->prev_arena->next_arena == curr_arena);
        }
    }
