...
#define LOGGING_PREAMBLE_WIDTH \
    (53 + LOGGING_THREADNAME_WIDTH + LOGGING_FILENAME_WIDTH)
...
void file_reopen(void* user_data) {
    ...
    if (!file_abs->fp || (ret = stat(file_abs->path, &st)) == -1 ||
        ...
        if (!file_abs->fp) {
            LOG_F(INFO, "Reopening file '%s' due to previous error",
                  file_abs->path);
        } else if (ret < 0) {
            const auto why = errno_as_text();
            LOG_F(INFO, "Reopening file '%s' due to '%s'", file_abs->path,
                  why.c_str());
        } else {
            LOG_F(INFO, "Reopening file '%s' due to file changed",
                  file_abs->path);
        }
        // try reopen current file.
        if (!create_directories(file_abs->path)) {
            LOG_F(ERROR, "Failed to create directories to '%s'",
                  file_abs->path);
        }
    ...
    for (int arg_it = 1; arg_it < argc; ++arg_it) {
        ...
        if (strncmp(cmd, verbosity_flag, arg_len) == 0 &&
            ...
            if (value_str[0] == '\0') {
                ...
                arg_it += 1;
                CHECK_LT_F(arg_it, argc, "Missing verbosiy level after %s",
                           verbosity_flag);
...
#define getcwd _getcwd
    ...
    if (!getcwd(s_current_dir, sizeof(s_current_dir))) {
        ...
        LOG_F(WARNING, "Failed to get current working directory: %s",
              error_text.c_str());
    }
...
bool add_file(const char* path_in, FileMode mode, Verbosity verbosity) {
    ...
    LOG_F(INFO, "Logging to '%s', mode: '%s', verbosity: %d", path, mode_str,
          verbosity);
    return true;
}
...
char* get_thread_name_win32() {
    __declspec(thread) static char thread_name[LOGGING_THREADNAME_WIDTH + 1] = {
        0};
...
void set_thread_name(const char* name) {
    ...
    strncpy_s(get_thread_name_win32(), LOGGING_THREADNAME_WIDTH + 1, name,
              _TRUNCATE);
...
#if LOGGING_VERBOSE_SCOPE_ENDINGS
        auto duration_sec = (now_ns() - _start_time_ns) / 1e9;
        auto buff = textprintf("%.*f s: %s", LOGGING_SCOPE_TIME_PRECISION,
                               duration_sec, _name);
    ...
    if (!stack.empty()) {
        ...
        for (auto entry : stack) {
            ...
            auto prefix = textprintf(
                "[ErrorContext] %*s:%-5u %-20s ", LOGGING_FILENAME_WIDTH,
