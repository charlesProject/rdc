/*
Logging logging library for C++, by Emil Ernerfeldt.
www.github.com/emilk/logging
If you find Logging useful, please let me know on twitter or in a mail!
Twitter: @ernerfeldt
...
# Inspiration
    Much of Logging was inspired by GLOG, https://code.google.com/p/google-glog/.
...
# Version history
    ...
    * Version 1.2.3 - 2016-05-16 - Log current working directory in logging::init().
    * Version 1.2.4 - 2016-05-18 - Custom replacement for -v in logging::init() by bjoernpollex
    ...
    * Version 1.3.1 - 2016-07-20 - Add LOGGING_UNSAFE_SIGNAL_HANDLER to toggle stacktrace on signals.
    * Version 1.3.2 - 2016-07-20 - Add logging::arguments()
    * Version 1.4.0 - 2016-09-15 - Semantic versioning + add logging::create_directories
    * Version 1.4.1 - 2016-09-29 - Customize formating with LOGGING_FILENAME_WIDTH
    * Version 1.5.0 - 2016-12-22 - LOGGING_USE_FMTLIB by kolis and LOGGING_WITH_FILEABS by scinart
    * Version 1.5.1 - 2017-08-08 - Terminal colors on Windows 10 thanks to looki
    * Version 1.6.0 - 2018-01-03 - Add LOGGING_RTTI and LOGGING_STACKTRACES settings
    * Version 1.7.0 - 2018-01-03 - Add ability to turn off the preamble with logging::g_preamble
...
# Compiling
    Just include <logging.hpp> where you want to use Logging.
...
# Usage
    ...
    logging::add_file("everything.log", logging::Append, logging::Verbosity_MAX);

    // Only log INFO, WARNING, ERROR and FATAL to "latest_readable.log":
    logging::add_file("latest_readable.log", logging::Truncate, logging::Verbosity_INFO);

    // Only show most relevant things on stderr:
    logging::g_stderr_verbosity = 1;

    // Or just go with what Logging suggests:
    char log_path[PATH_MAX];
    logging::suggest_log_path("~/logging/", log_path, sizeof(log_path));
    logging::add_file(log_path, logging::FileMode::Truncate, logging::Verbosity_MAX);
    ...
    DCHECK_F(expensive_check(x)); // Only checked #if LOGGING_DEBUG_CHECKS
    DLOG_F("Only written in debug-builds");

    // Turn off writing to stderr:
    logging::g_stderr_verbosity = logging::Verbosity_OFF;

    // Turn off writing err/warn in red:
    logging::g_colorlogtostderr = false;

    // Throw exceptions instead of aborting on CHECK fails:
    logging::set_fatal_handler([](const logging::Message& message){
        throw std::runtime_error(std::string(message.prefix) + message.message);
    });

    If you prefer logging with streams:

    #define LOGGING_WITH_STREAMS 1
    #include <logging.hpp>
    ...
    LOG_S(INFO) << "Look at my custom object: " << a.cross(b);
    CHECK_EQ_S(pi, 3.14) << "Maybe it is closer to " << M_PI;

    Before including <logging.hpp> you may optionally want to define the following to 1:

    LOGGING_DEBUG_LOGGING (default 1 #if !NDEBUG, else 0):
        Enables debug versions of logging statements.

    LOGGING_DEBUG_CHECKS (default 1 #if !NDEBUG, else 0):
        Enables debug versions of checks.

    LOGGING_REDEFINE_ASSERT (default 0):
        Redefine "assert" call Logging version (!NDEBUG only).

    LOGGING_WITH_STREAMS (default 0):
    ...
    LOGGING_REPLACE_GLOG (default 0):
        Make Logging mimic GLOG as close as possible,
        including #defining LOG, CHECK, VLOG_IS_ON etc.
        LOGGING_REPLACE_GLOG implies LOGGING_WITH_STREAMS.

    LOGGING_UNSAFE_SIGNAL_HANDLER (default 1):
        Make Logging try to do unsafe but useful things,
        like printing a stack trace, when catching signals.
        This may lead to bad things like deadlocks in certain situations.

    LOGGING_USE_FMTLIB (default 0):
        Use fmtlib formatting. See https://github.com/fmtlib/fmt
        This will make logging.hpp depend on <fmt/format.h>
        You will need to link against `fmtlib` or use the `FMT_HEADER_ONLY` preprocessor definition.
        Feature by kolis (https://github.com/emilk/logging/pull/22)

    LOGGING_WITH_FILEABS (default 0):
        When LOGGING_WITH_FILEABS is defined, a check of file change will be performed on every call to file_log.
        ...
        Feature by scinart (https://github.com/emilk/logging/pull/23).

    LOGGING_STACKTRACES (default 1 on supported platforms):
        Print stack traces on abort.

    LOGGING_RTTI (try to detect automatically by default):
        Set to 0 if your platform does not support runtime type information (-fno-rtti).

    You can also configure:
    logging::g_flush_interval_ms:
        If set to zero Logging will flush on every line (unbuffered mode).
        Else Logging will flush outputs every g_flush_interval_ms milliseconds (buffered mode).
...
#ifndef LOGGING_HAS_DECLARED_FORMAT_HEADER
#define LOGGING_HAS_DECLARED_FORMAT_HEADER

// Semantic versioning. Logging version can be printed with printf("%d.%d.%d", LOGGING_VERSION_MAJOR, LOGGING_VERSION_MINOR, LOGGING_VERSION_PATCH);
#define LOGGING_VERSION_MAJOR 1
#define LOGGING_VERSION_MINOR 8
#define LOGGING_VERSION_PATCH 0
...
// ----------------------------------------------------------------------------
...
#ifndef LOGGING_SCOPE_TEXT_SIZE
    ...
    #define LOGGING_SCOPE_TEXT_SIZE 196
#endif

#ifndef LOGGING_FILENAME_WIDTH
    // Width of the column containing the file name
    #define LOGGING_FILENAME_WIDTH 23
#endif

#ifndef LOGGING_THREADNAME_WIDTH
    // Width of the column containing the thread name
    #define LOGGING_THREADNAME_WIDTH 16
#endif

#ifndef LOGGING_CATCH_SIGABRT
    // Should Logging catch SIGABRT to print stack trace etc?
    #define LOGGING_CATCH_SIGABRT 1
#endif

#ifndef LOGGING_REDEFINE_ASSERT
    #define LOGGING_REDEFINE_ASSERT 0
#endif

#ifndef LOGGING_WITH_STREAMS
    #define LOGGING_WITH_STREAMS 0
#endif

#ifndef LOGGING_REPLACE_GLOG
    #define LOGGING_REPLACE_GLOG 0
#endif

#if LOGGING_REPLACE_GLOG
    #undef LOGGING_WITH_STREAMS
    #define LOGGING_WITH_STREAMS 1
#endif

#ifndef LOGGING_UNSAFE_SIGNAL_HANDLER
    #define LOGGING_UNSAFE_SIGNAL_HANDLER 1
#endif
...
#ifndef LOGGING_USE_FMTLIB
    #define LOGGING_USE_FMTLIB 0
#endif

#ifndef LOGGING_WITH_FILEABS
    #define LOGGING_WITH_FILEABS 0
#endif

#ifndef LOGGING_RTTI
#if defined(__clang__)
    #if __has_feature(cxx_rtti)
        #define LOGGING_RTTI 1
    #endif
...
#   define LOGGING_ANONYMOUS_VARIABLE(str) LOGGING_CONCATENATE(str, __LINE__)
    ...
    #define LOGGING_PRINTF_LIKE(fmtarg, firstvararg) __attribute__((__format__ (__printf__, fmtarg, firstvararg)))
...
namespace logging
{
    ...
    Text textprintf(LOGGING_FORMAT_STRING_TYPE format, ...) LOGGING_PRINTF_LIKE(1, 2);
...
#undef MAX
    ...
    Set to loggingr::Verbosity_OFF to write nothing to stderr.
    ...
    */
    ...
    /*  Should be called from the main thread.
        ...
        logging::init() will look for arguments meant for logging and remove them.
        Arguments meant for logging are:
    ...
    */
    ...
    /* Given a prefix of e.g. "~/logging/" this might return
       "/home/your_username/logging/app_name/20151017_161503.123.log"
    ...
    */
    ...
    void log(Verbosity verbosity, const char* file, unsigned line, LOGGING_FORMAT_STRING_TYPE format, fmt::ArgList args);
    FMT_VARIADIC(void, log, Verbosity, const char*, unsigned, LOGGING_FORMAT_STRING_TYPE)

    // Log without any preamble or indentation.
    void raw_log(Verbosity verbosity, const char* file, unsigned line, LOGGING_FORMAT_STRING_TYPE format, fmt::ArgList args);
    FMT_VARIADIC(void, raw_log, Verbosity, const char*, unsigned, LOGGING_FORMAT_STRING_TYPE)
#else // LOGGING_USE_FMTLIB?
    // Actual logging function. Use the LOG macro instead of calling this directly.
    void log(Verbosity verbosity, const char* file, unsigned line, LOGGING_FORMAT_STRING_TYPE format, ...) LOGGING_PRINTF_LIKE(4, 5);

    // Log without any preamble or indentation.
    void raw_log(Verbosity verbosity, const char* file, unsigned line, LOGGING_FORMAT_STRING_TYPE format, ...) LOGGING_PRINTF_LIKE(4, 5);
#endif // !LOGGING_USE_FMTLIB
    ...
    public:
        ...
        LogScopeRAII(Verbosity verbosity, const char* file, unsigned line, LOGGING_FORMAT_STRING_TYPE format, ...) LOGGING_PRINTF_LIKE(5, 6);
        ~LogScopeRAII();
    ...
    private:
        ...
        char        _name[LOGGING_SCOPE_TEXT_SIZE];
    };
    ...
    LOGGING_NORETURN void log_and_abort(int stack_trace_skip, const char* expr, const char* file, unsigned line, LOGGING_FORMAT_STRING_TYPE format, ...) LOGGING_PRINTF_LIKE(5, 6);
    LOGGING_NORETURN void log_and_abort(int stack_trace_skip, const char* expr, const char* file, unsigned line);
    ...
    /* Returns the thread name for this thread.
       On OSX this will return the system thread name (settable from both within and without Logging).
    ...
    */
    ...
    is set, Logging will write them to stderr to make errors in red, etc.
    ...
    This means if you intend to use them functions you should either:
        * Use them on the stderr/stdout directly (bypass Logging).
        * Don't add file outputs to Logging.
    ...
    */
    #define ERROR_CONTEXT(descr, data)                                             \
        const logging::EcEntryData<logging::make_ec_type<decltype(data)>::type>      \
            LOGGING_ANONYMOUS_VARIABLE(error_context_scope_)(                       \
...
/*
    #define ERROR_CONTEXT(descr, data)                                 \
        const auto LOGGING_ANONYMOUS_VARIABLE(error_context_scope_)(    \
...
#define VLOG_SCOPE_F(verbosity, ...)                                                               \
    logging::LogScopeRAII LOGGING_ANONYMOUS_VARIABLE(error_context_RAII_) =                          \
    ((verbosity) > logging::current_verbosity_cutoff()) ? logging::LogScopeRAII() :                  \
...
#define CHECK_WITH_INFO_F(test, info, ...)                                                         \
    LOGGING_PREDICT_TRUE((test) == true) ? (void)0 : logging::log_and_abort(0, "CHECK FAILED:  " info "  ", __FILE__,      \
...
#define CHECK_OP_F(expr_left, expr_right, op, ...)                                                 \
    ...
    {                                                                                              \
        ...
        if (! LOGGING_PREDICT_TRUE(val_left op val_right))                                          \
        {                                                                                          \
            auto str_left = logging::format_value(val_left);                                        \
            auto str_right = logging::format_value(val_right);                                      \
            auto fail_info = logging::textprintf("CHECK FAILED:  %s %s %s  (%s %s %s)  ",           \
...
#ifndef LOGGING_DEBUG_LOGGING
    #ifndef NDEBUG
        #define LOGGING_DEBUG_LOGGING 1
    #else
        #define LOGGING_DEBUG_LOGGING 0
    #endif
#endif

#if LOGGING_DEBUG_LOGGING
...
#define CHECK_GE_F(a, b, ...) CHECK_OP_F(a, b, >=, ##__VA_ARGS__)
...
#ifndef LOGGING_DEBUG_CHECKS
    #ifndef NDEBUG
        #define LOGGING_DEBUG_CHECKS 1
    #else
        #define LOGGING_DEBUG_CHECKS 0
    #endif
#endif

#if LOGGING_DEBUG_CHECKS
    ...
    #undef assert
        ...
        #define assert(test)
    #endif
#endif // LOGGING_REDEFINE_ASSERT

#endif // LOGGING_HAS_DECLARED_FORMAT_HEADER
...
#if LOGGING_WITH_STREAMS
#ifndef LOGGING_HAS_DECLARED_STREAMS_HEADER
#define LOGGING_HAS_DECLARED_STREAMS_HEADER

/* This file extends logging to enable std::stream-style logging, a la Glog.
   It's an optional feature behind the LOGGING_WITH_STREAMS settings
...
namespace logging
{
    ...
    std::string strprintf(LOGGING_FORMAT_STRING_TYPE format, ...) LOGGING_PRINTF_LIKE(1, 2);

    // Like vsprintf, but returns the formated text.
    std::string vstrprintf(LOGGING_FORMAT_STRING_TYPE format, va_list) LOGGING_PRINTF_LIKE(1, 0);
    ...
    public:
        ...
        LOGGING_NORETURN ~AbortLogger() noexcept(false);
...
#define CHECK_WITH_INFO_S(cond, info)                                                              \
    LOGGING_PREDICT_TRUE((cond) == true)                                                            \
        ? (void)0                                                                                  \
...
#endif // LOGGING_REPLACE_GLOG

#endif // LOGGING_WITH_STREAMS

#endif // LOGGING_HAS_DECLARED_STREAMS_HEADER
...
This will define all the Logging functions so that the linker may find them.
*/

#if defined(LOGGING_IMPLEMENTATION) && !defined(LOGGING_HAS_BEEN_IMPLEMENTED)
#define LOGGING_HAS_BEEN_IMPLEMENTED

#define LOGGING_PREAMBLE_WIDTH (53 + LOGGING_THREADNAME_WIDTH + LOGGING_FILENAME_WIDTH)
...
// TODO: use defined(_POSIX_VERSION) for some of these things?
    ...
    #define LOGGING_PTHREADS    0
    #define LOGGING_WINTHREADS  1
    #ifndef LOGGING_STACKTRACES
        #define LOGGING_STACKTRACES 0
    #endif
#elif defined(__rtems__)
    #define LOGGING_PTHREADS    1
    #define LOGGING_WINTHREADS  0
    #ifndef LOGGING_STACKTRACES
        #define LOGGING_STACKTRACES 0
    #endif
#else
    ...
    #define LOGGING_WINTHREADS  0
    #ifndef LOGGING_STACKTRACES
        #define LOGGING_STACKTRACES 1
    #endif
#endif

#if LOGGING_STACKTRACES
    #include <cxxabi.h>    // for __cxa_demangle
    #include <dlfcn.h>     // for dladdr
    #include <execinfo.h>  // for backtrace
#endif // LOGGING_STACKTRACES
    ...
    #include <pthread.h>
        ...
        /* On Linux, the default thread name is the same as the name of the binary.
           ...
           For this reason, Logging use the pthread Thread Local Storage
           for storing thread names on Linux. */
        #define LOGGING_PTLS_NAMES 1
    #endif
#endif

#if LOGGING_WINTHREADS
    #ifndef _WIN32_WINNT
...
#ifndef LOGGING_PTLS_NAMES
   #define LOGGING_PTLS_NAMES 0
#endif
...
#define VTSEQ(ID) ("\e[" #ID "m")
    ...
    void init(int& argc, char* argv[], const char* verbosity_flag)
    {
        ...
        #if LOGGING_PTLS_NAMES || LOGGING_WINTHREADS
            set_thread_name("main thread");
        #elif LOGGING_PTHREADS
        ...
        #endif // LOGGING_PTHREADS

        if (g_stderr_verbosity >= Verbosity_INFO) {
            if (g_preamble) {
                char preamble_explain[LOGGING_PREAMBLE_WIDTH];
    ...
    {
        __declspec( thread ) static char thread_name[LOGGING_THREADNAME_WIDTH + 1] = {0};
        return &thread_name[0];
    }
#endif // LOGGING_WINTHREADS
    ...
    void set_thread_name(const char* name)
    {
            ...
            #endif
        #elif LOGGING_WINTHREADS
            strncpy_s(get_thread_name_win32(), LOGGING_THREADNAME_WIDTH + 1, name, _TRUNCATE);
        #else // LOGGING_PTHREADS
            (void)name;
        #endif // LOGGING_PTHREADS
    }
...
#else // !LOGGING_WINTHREADS && !LOGGING_WINTHREADS
        buffer[0] = 0;
#endif
    ...
    static void print_preamble_header(char* out_buff, size_t out_buff_size)
    {
        ...
        if (g_preamble_thread && pos < out_buff_size) {
            pos += snprintf(out_buff + pos, out_buff_size - pos, "[%-*s]", LOGGING_THREADNAME_WIDTH, " thread name/id");
        }
        if (g_preamble_file && pos < out_buff_size) {
            pos += snprintf(out_buff + pos, out_buff_size - pos, "%*s:line  ", LOGGING_FILENAME_WIDTH, "file");
        }
    ...
    static void print_preamble(char* out_buff, size_t out_buff_size, Verbosity verbosity, const char* file, unsigned line)
    {
        ...
        char thread_name[LOGGING_THREADNAME_WIDTH + 1] = {0};
        get_thread_name(thread_name, LOGGING_THREADNAME_WIDTH + 1, true);
        ...
        if (g_preamble_thread && pos < out_buff_size) {
            pos += snprintf(out_buff + pos, out_buff_size - pos, "[%-*s]",
                           LOGGING_THREADNAME_WIDTH, thread_name);
        }
        if (g_preamble_file && pos < out_buff_size) {
            char shortened_filename[LOGGING_FILENAME_WIDTH + 1];
            snprintf(shortened_filename, LOGGING_FILENAME_WIDTH + 1, "%s", file);
            pos += snprintf(out_buff + pos, out_buff_size - pos, "%*s:%-5u ",
                           LOGGING_FILENAME_WIDTH, shortened_filename, line);
        }
        if (g_preamble_verbose && pos < out_buff_size) {
    ...
    static void log_message(int stack_trace_skip, Message& message, bool with_indentation, bool abort_if_fatal)
    {
        ...
        if (message.verbosity == Verbosity_FATAL) {
            ...
            if (abort_if_fatal) {
#if LOGGING_CATCH_SIGABRT && !defined(_WIN32)
    ...
    {
        char preamble_buff[LOGGING_PREAMBLE_WIDTH];
    ...
    {
        auto message = _ss.str();
        logging::log_and_abort(1, _expr, _file, _line, "%s", message.c_str());
    }
    ...
    #ifdef __APPLE__
        #define LOGGING_THREAD_LOCAL __thread
    #else
        #define LOGGING_THREAD_LOCAL thread_local
    #endif
    static LOGGING_THREAD_LOCAL ECPtr thread_ec_ptr = nullptr;
        ...
        if (!stack.empty()) {
            ...
            for (auto entry : stack) {
                ...
                auto prefix = textprintf("[ErrorContext] %*s:%-5u %-20s ",
                    LOGGING_FILENAME_WIDTH, filename(entry->_file), entry->_line, description.c_str());
...
namespace logging
{
    ...
    void signal_handler(int signal_number, siginfo_t*, void*)
    {
        ...
        */
        ...
        write_to_stderr("\n");
        write_to_stderr("Logging caught a signal: ");
...
#if LOGGING_UNSAFE_SIGNAL_HANDLER
        // --------------------------------------------------------------------
        ...
        */
        ...
        try {
            ...
            write_to_stderr("Exception caught and ignored by Logging signal handler.\n");
        }
        flush();

        // --------------------------------------------------------------------
#endif // LOGGING_UNSAFE_SIGNAL_HANDLER
