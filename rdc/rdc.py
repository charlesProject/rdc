...
def broadcast(data, root):
    ...
    if root == rank:
        ...
        length.value = len(s)
    # run first broadcast
    _LIB.RdcBroadcast(ctypes.byref(length),
                        ctypes.sizeof(ctypes.c_ulong), root)
    if root != rank:
        dptr = (ctypes.c_char * length.value)()
        # run second
        _LIB.RdcBroadcast(ctypes.cast(dptr, ctypes.c_void_p),
                            length.value, root)
        data = pickle.loads(dptr.raw)
        del dptr
    else:
        _LIB.RdcBroadcast(ctypes.cast(ctypes.c_char_p(s), ctypes.c_void_p),
                            length.value, root)
        del s
    return data
...
def allreduce(data, op, prepare_fun=None):
    ...
    if prepare_fun is None:
        _LIB.RdcAllreduce(buf.ctypes.data_as(ctypes.c_void_p),
